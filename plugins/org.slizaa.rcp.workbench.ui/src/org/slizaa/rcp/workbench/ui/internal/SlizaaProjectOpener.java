...
public class SlizaaProjectOpener {
  ...
  public static void openProject(final SlizaaProject slizaaProject, boolean parse) {
    ...
    openProjectInternal(slizaaProject, parse);

    final long duration = System.currentTimeMillis() - start;

    Activator.getDefault().getLog().log(new Status(Status.INFO, Activator.PLUGIN_ID, "Opening slizaa Project '"
        + slizaaProject.getProject().getName() + "' took " + duration + "ms (" + (duration / 1000) + "s)"));
  ...
  private static void openProjectInternal(final SlizaaProject bundleMakerProject, boolean parse) {
    ...
    ParseSlizaaProjectRunnable.parseProject(bundleMakerProject, parse);
    ...
    Activator.getDefault().getLog().log(new Status(Status.INFO, Activator.PLUGIN_ID, "Fire Project open took "
        + fireProjectOpenedStartDuration + "ms (" + (fireProjectOpenedStartDuration / 1000) + "s)"));
