...
public class SlizaaPerspectiveAdapter extends PerspectiveAdapter {
  ...
  public SlizaaPerspectiveAdapter(EModelService modelService, MApplication application, EPartService partService) {
    ...
    //
    this._view2Perspective = CacheBuilder.newBuilder().build(new CacheLoader<String, Map<String, PartStateHolder>>() {
      @Override
    ...
    for (MWindow mWindow : modelService.findElements(application, null, MWindow.class, null)) {
      this._currentPerspective = modelService.getActivePerspective(mWindow);
    }
  }
  ...
  public void perspectiveActivated(IWorkbenchPage page, IPerspectiveDescriptor perspectiveDescriptor) {
    ...
    //
    List<MPerspective> mPerspectives = this._modelService.findElements(this._application, perspectiveDescriptor.getId(),
        MPerspective.class, null);

    //
    MPerspective newPerspective = mPerspectives.get(0);
    onSwitchPerspective(newPerspective, this._currentPerspective);
    this._currentPerspective = newPerspective;
  }
  ...
  private void onSwitchPerspective(MPerspective newPerspective, MPerspective oldPerspective) {
    ...
    List<MPartStack> partStacks = this._modelService.findElements(newPerspective, null, MPartStack.class,
        Arrays.asList(new String[] { "EditorStack" }));

    Map<String, PartStateHolder> oldPerspectiveViewMap = this._view2Perspective
        .getUnchecked(oldPerspective.getElementId());
    Map<String, PartStateHolder> newPerspectiveViewMap = this._view2Perspective
        .getUnchecked(newPerspective.getElementId());
    ...
    for (MStackElement stackElement : partStacks.get(0).getChildren()) {
      if (stackElement instanceof MPlaceholder) {
        ...
        if (partStateHolder.isToBeRendered()) {
          ...
          this._partService.hidePart((MPart) placeholder.getRef());
        }
      }
    }
    ...
    if (selectedPart != null) {
      this._partService.showPart(selectedPart, PartState.ACTIVATE);
    }

    //
    if (Activator.CROSS_REFERENCER_PERSPECTIVE.equals(newPerspective.getElementId())) {
      this._partService.showPart("org.slizaa.ui.xref.XRefPart", PartState.ACTIVATE);
    }
  }
}