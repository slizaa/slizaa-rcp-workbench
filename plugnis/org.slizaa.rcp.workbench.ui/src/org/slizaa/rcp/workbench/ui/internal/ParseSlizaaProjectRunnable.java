 ...
 *
 * @see SlizaaProjectOpener for a convienence class that not only parses the project but also runs all required actions
 *      for the UI handling
 *
...
public class ParseSlizaaProjectRunnable implements IRunnableWithProgress {
  ...
  private final SlizaaProject _bundleMakerProject;
   ...
   */
  public ParseSlizaaProjectRunnable(SlizaaProject bundleMakerProject, boolean parse) {
  ...
  public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
    try {
      // this._bundleMakerProject.initialize(monitor);
      ...
      // parse the project
      if (this._parse) {
        this._bundleMakerProject.parse(monitor);
      } else {
        this._bundleMakerProject.startAndConnectDatabase(monitor);
      }
   ...
   */
  public static void parseProject(SlizaaProject bundleMakerProject, boolean parse) {
    // Create runnable
    ParseSlizaaProjectRunnable runnable = new ParseSlizaaProjectRunnable(bundleMakerProject, parse);
